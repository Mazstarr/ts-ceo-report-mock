...
//     return monthlySales;
// }
function getMonthlyRecord(data, dateProperty) {
    var currentDate = new Date();
    var currentYear = currentDate.getFullYear();
    var currentMonth = currentDate.getMonth() - 1; //remember to remove -1 to get for current month
    return data.filter(function (record) {
        var recordDate = new Date(record[dateProperty]);
...
function revenueAndSalesTrends() {
    ...
    var sortedPayments = payments_df.sort(function (a, b) {
        return a.reporting_date.getTime() - b.reporting_date.getTime();
    });
    var recentPayments = sortedPayments.filter(function (payment) { return payment.reporting_date >= lastYear; });
    var revenueTrends = recentPayments.reduce(function (acc, payment) {
        var paymentDate = (0, date_fns_1.format)(payment.reporting_date, 'yyyy-MM');
        if (!acc[paymentDate]) {
...
function customerGrowthAndRetention() {
    ...
    var sortedCustomers = customers_df.sort(function (a, b) {
        return a.customer_created_at.getTime() - b.customer_created_at.getTime();
    });
    ...
    var startCustomers = customers_df.filter(function (customer) { return customer.customer_created_at <= oneYearAgo; }).length;
    var newCustomers = customers_df.filter(function (customer) { return customer.customer_created_at >= oneYearAgo; }).length;
    var endCustomers = customers_df.filter(function (customer) { return customer.datetime_created_at_local >= sixMonthsAgo; }).length;
...
function disputeAnalysis() {
    var disputesOpen = disputes_df.filter(function (dispute) { return dispute.dispute_status === "Open"; }).length;
    var disputesResolvedLastMonth = disputes_df.filter(function (dispute) {
        return dispute.dispute_status === "Resolved" &&
            dispute.dispute_created_at_date >= (0, date_fns_1.subMonths)(new Date(), 1);
    }).length;
    var resolvedDisputes = disputes_df.filter(function (dispute) { return dispute.dispute_status === "Resolved"; });
    var meanTimeToResolution = 0;
    if (resolvedDisputes.length > 0) {
        var totalDays = resolvedDisputes.reduce(function (acc, dispute) {
            var createdDate = new Date(dispute.dispute_created_at_date);
            var resolvedDate = new Date(dispute.dispute_resolved_at_date);
...
function subscriptionPerformance() {
    var sortedPayments = subscription_df.sort(function (a, b) {
        return a.dw_modified.getTime() - b.dw_modified.getTime();
    });
    var subscriptionItems = getMonthlyRecord(subscription_df, 'dw_modified');
    var totalSubscriptions = subscriptionItems.length;
    var subscriptionVolume = subscriptionItems.reduce(function (sum, sale) { return sum + sale.amount_subscription; }, 0);
    var uniqueSubscribers = new Set(subscriptionItems.map(function (payment) { return payment.dimcustomerid; })).size;
    var subscriptionHistory = sortedPayments.reduce(function (acc, payment) {
...
function peakShoppingTimes() {
    var paymentHours = payments_df.map(function (payment) { return payment.reporting_date.getHours(); });
...
function customerSegmentation() {
    var today = new Date();
    var startOfMonth = new Date(today.getFullYear(), (today.getMonth() - 1), 1); //remove -1 to get for current month
    var endOfMonth = new Date(today.getFullYear(), (today.getMonth() - 1) + 1, 0); //remove -1 to get for current month
    // Identify new customers (added this month)
    var newCustomers = customers_df.filter(function (customer) {
    ...
    var returningCustomers = customers_df.filter(function (customer) {
        return customer.customer_created_at && customer.customer_created_at < startOfMonth &&
    ...
    var nonReturningCustomers = customers_df.filter(function (customer) {
        return customer.customer_created_at && customer.customer_created_at < startOfMonth &&
            (!customer.datetime_created_at_local || customer.datetime_created_at_local < startOfMonth);
    });
    var totalCustomers = customers_df.length;
    // Calculations
    var percentageReturning = (returningCustomers.length / totalCustomers) * 100;
    var percentageNew = (newCustomers.length / totalCustomers) * 100;
    var percentageNonReturning = (nonReturningCustomers.length / totalCustomers) * 100;
...
function performanceComparison() {
    ...
    var currentMonthSales = payments_df
        .filter(function (payment) {
        var paymentDate = new Date(payment.reporting_date);
        return paymentDate.getFullYear() === currentYear && paymentDate.getMonth() === currentMonth;
    })
        .reduce(function (sum, payment) { return sum + parseFloat(payment.transaction_amount); }, 0);
    var lastMonthSales = payments_df
        .filter(function (payment) {
        var paymentDate = new Date(payment.reporting_date);
...
function calculateSuccessRate() {
    var monthlySales = getMonthlyRecord(payments_df, 'reporting_date');
    var totalPayments = monthlySales.length;
    var totalVolume = monthlySales.reduce(function (sum, sale) { return sum + parseFloat(sale.transaction_amount); }, 0);
    var successfulPayments = monthlySales.filter(function (payment) { return Boolean(payment.is_successful); }).length;
    var failedPayments = monthlySales.filter(function (payment) { return !Boolean(payment.is_successful); }).length;
...
function loadData() {
    ...
    return Promise.all([
        db_connection_1.databaseRepo.getWhere(constants_1.TABLES.TRANSACTIONS, { merchant_id: merchant_id }).then(function (data) {
            payments_df = data;
        }),
        db_connection_1.databaseRepo.getWhere(constants_1.TABLES.CUSTOMERS, { merchant_id: merchant_id }, undefined, undefined, undefined, rawQuery, [constants_1.TABLES.CUSTOMERS, 'merchant_id', merchant_id]).then(function (data) {
            customers_df = data;
        }),
        db_connection_1.databaseRepo.getWhere(constants_1.TABLES.SUBSCRIPTIONS, { merchant_id: merchant_id }, undefined, 'datetime_created_at')
            .then(function (data) {
            subscription_df = data;
        }),
        db_connection_1.databaseRepo.getWhere(constants_1.TABLES.DISPUTES, { merchant_id: merchant_id }, undefined, 'dispute_created_at_date').then(function (data) {
            disputes_df = data;
        }),
    ]).then(function () { return __awaiter(_this, void 0, void 0, function () {
        var da, sp, psp, rst, pc, sr, cgr, csg;
        return __generator(this, function (_a) {
            da = disputeAnalysis();
            sp = subscriptionPerformance();
            psp = peakShoppingTimes() //no timestamp in data being returned from redshift
            ;
